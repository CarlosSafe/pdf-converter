...
1. **NPM Script**
   ...
   - Add a script entry to your `package.json`:
     ...
     {
       "scripts": {
          "lint:css:check": "stylelint \"../app/core/src/main/**/*.css\" --config ../.stylelintrc.json",
          "lint:css:fix": "stylelint \"../app/core/src/main/**/*.css\" --config .stylelintrc.json --fix"
       }
     }
     ```
   - Run the linter:
...
2. **CLI Usage**
   ...
   - Lint all CSS files:
     ```bash
     npx stylelint ../app/core/src/main/**/*.css
     ```
   - Apply automatic fixes:
     ```bash
     npx stylelint "../app/core/src/main/**/*.css" --fix
     ```
